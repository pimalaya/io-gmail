...
struct GmailMessageImportRequest<'a> {
    raw: String,
...
impl GmailMessageImport {
    pub fn new(
        ...
        let url = Url::parse(GMAIL_API_BASE)?.join(&format!("users/{user_id}/messages/import"))?;
        let body = GmailMessageImportRequest {
...
impl GmailCoroutine for GmailMessageImport {
    ...
    fn resume(&mut self, arg: Option<&[u8]>) -> GmailCoroutineState<Self::Yield, Self::Return> {
        trace!("message-import: {}", self.state);
